...
public final class PatientLinkSpecifications {
    ...
    public static Specification<PatientOrganizationLink> matching(UUID organizationId, UUID clinicUnitId,
        ...
        return (root, query, builder) -> {
            ...
            predicate = appendIfPresent(predicate, builder, patient.get("id"), filter.id());
            predicate = like(predicate, builder, patient.get("name"), filter.normalized(filter.name()));
            predicate = appendIfPresent(predicate, builder, patient.get("birthDate"), filter.birthDate());
            predicate = appendIfPresent(predicate, builder, patient.get("active"), filter.active());
            predicate = appendIfPresent(predicate, builder, patient.get("gender"), filter.gender());
            predicate = like(predicate, builder, patient.get("taxId"), filter.normalized(filter.taxId()));
            predicate = appendIfPresent(predicate, builder, patient.get("identificationType"), filter.identificationType());
            ...
            predicate = appendIfPresent(predicate, builder, patient.join("address").get("id"), filter.addressId());
    ...
    private static <T> jakarta.persistence.criteria.Predicate appendIfPresent(jakarta.persistence.criteria.Predicate predicate,
