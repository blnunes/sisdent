...
public interface AppointmentRepository extends JpaRepository<Appointment,Long> {
 ...
 boolean existsByClinicUnit_Id(Long clinicUnitId);
 ...
 @EntityGraph(attributePaths={"organization","clinicUnit","patientLink","patientLink.patient","practitioner"}) @Query("select a from Appointment a where a.organization.globalId=:organizationId and (:clinicUnitId is null or a.clinicUnit.globalId=:clinicUnitId) and a.endAt >= :from") Page<Appointment> findFrom(UUID organizationId,UUID clinicUnitId,Instant from,Pageable page);
