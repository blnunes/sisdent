...
const MINIMUM_INSTRUCTION_COVERAGE = 80;
...
process.stdin.on('data', (chunk) => {
  ...
  while ((newline = buffer.indexOf('\n')) >= 0) {
    ...
    if (request.method === 'initialize') {
      respond({ jsonrpc: '2.0', id: request.id, result: { protocolVersion: request.params?.protocolVersion ?? '2025-03-26', capabilities: { tools: {} }, serverInfo: { name: 'sisdent-quality-gate', version: '1.0.0' }, instructions: 'Before completing Java work, call verify_quality. Add focused tests for every changed behaviour; fix coverage and Sonar findings rather than suppressing them. For Angular and TypeScript, never render or interpolate an unknown API value with String(value), template interpolation, or implicit coercion. Narrow values to a supported primitive first; for UI text, use shared/text-value.ts textValue(value, fallback) so null, arrays, and malformed objects render the explicit fallback rather than [object Object]. Add regression tests for malformed API values whenever a new UI mapping is introduced. NON-NEGOTIABLE: constructors and methods with more than seven parameters are unacceptable. Do not suppress Sonar java:S107; redesign the API with cohesive value objects, commands, or builders and cover the refactor with focused tests. Format every new or refactored Java class and frontend component with this project\'s IntelliJ code style: one declaration per line, standard indentation, braces for conditional blocks, and no compressed one-line declarations, methods, or control flow. Sonar java:S2629: guard every SLF4J log call whose arguments invoke methods or allocate values with the matching LOG.isTraceEnabled(), isDebugEnabled(), isInfoEnabled(), isWarnEnabled(), or isErrorEnabled() check; calculate those log-only values inside that guard. Prefer a method reference over an equivalent lambda when it is clearer, for example DentalProcedureResponse::name instead of procedure -> procedure.name(); keep the lambda when the method reference would be less readable.' } });
