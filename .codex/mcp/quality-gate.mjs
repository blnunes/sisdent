...
process.stdin.on('data', (chunk) => {
  ...
  while ((newline = buffer.indexOf('\n')) >= 0) {
    ...
    if (request.method === 'initialize') {
      respond({ jsonrpc: '2.0', id: request.id, result: { protocolVersion: request.params?.protocolVersion ?? '2025-03-26', capabilities: { tools: {} }, serverInfo: { name: 'sisdent-quality-gate', version: '1.0.0' }, instructions: 'Before completing Java work, call verify_quality. Add focused tests for every changed behaviour; fix coverage and Sonar findings rather than suppressing them. Sonar java:S2629: guard every SLF4J log call whose arguments invoke methods or allocate values with the matching LOG.isTraceEnabled(), isDebugEnabled(), isInfoEnabled(), isWarnEnabled(), or isErrorEnabled() check; calculate those log-only values inside that guard. Prefer a method reference over an equivalent lambda when it is clearer, for example DentalProcedureResponse::name instead of procedure -> procedure.name(); keep the lambda when the method reference would be less readable.' } });
