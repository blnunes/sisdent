...
export class OrganizationMutationGraphqlService {
  ...
  createClinicUnit(organizationId: string, input: { name: string }): Observable<ClinicUnit> {
    return this.graphql
      .query<{ createClinicUnit: ClinicUnit }>(
    ...
    const query = `mutation SavePractitioner($organizationId: ID!, $practitionerId: ID, $input: PractitionerMutationInput!) {
      ${operation}${
        practitionerId
          ...
          : '(organizationId: $organizationId, input: $input)'
      } { ${PRACTITIONER_FIELDS} }
    }`;
    return this.graphql
      .query<Record<string, Practitioner>>(query, {
        organizationId,
        practitionerId,
  ...
  deactivatePractitioner(organizationId: string, practitionerId: string): Observable<void> {
    return this.graphql
      .query<{ deactivatePractitioner: boolean }>(
        'mutation DeactivatePractitioner($organizationId: ID!, $practitionerId: ID!) { deactivatePractitioner(organizationId: $organizationId, practitionerId: $practitionerId) }',
