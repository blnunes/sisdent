...
describe('patient mappings', () => {
  it('maps a persisted patient to editable values', () => {
    const values = patientToForm({
      ...
      address: {
        ...
        street: 'Main',
        country: { code: 'PT' },
        administrativeDivision: { code: 'LX' },
      },
      documentIssuerCountry: { code: 'PT' },
      nationality: { code: 'BR' },
      specialities: [{ id: 2 }, { id: 5 }],
    });
  ...
  it('does not stringify malformed object values into editable fields', () => {
    const values = patientToForm({ name: { unexpected: true }, address: { street: ['invalid'] } });
  ...
  it('reuses a selected address and keeps only positive integer speciality ids', () => {
    const request = patientRequest({
      ...
      countryCode: 'PT',
      specialityIds: '2, -1, invalid, 3.5, 5',
  ...
  it('embeds a new address when no reusable address was selected', () => {
    const request = patientRequest({
      ...
      block: '',
      postalCode: '1000',
      administrativeDivisionName: 'Lisbon',
      administrativeDivisionCode: 'LX',
    ...
    expect(request['address']).toMatchObject({
      street: 'Main',
      city: 'Lisbon',
      administrativeDivision: { code: 'LX' },
      countryCode: 'PT',
