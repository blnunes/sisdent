...
export abstract class ResourceListController {
  ...
  load(): void {
    ...
    this.http.get<PageResponse<ResourceRecord>>(endpoint, { params: this.tableQuery.toHttpParams({ page: this.page(), size: this.pageSize(), sort: this.sort(), direction: this.sortDirection(), filters: this.filterValues() }) }).subscribe({
      next: (response) => { this.records.set(response.content); this.totalElements.set(response.totalElements); this.loading.set(false); },
      error: () => { this.error.set(true); this.loading.set(false); },
    });
  }

  changePage(event: DataTablePageEvent): void { this.page.set(event.pageIndex); this.pageSize.set(event.pageSize); this.load(); }
  changeSort(event: DataTableSortEvent): void {
    const next = this.tableQuery.nextSort({ page: this.page(), size: this.pageSize(), sort: this.sort(), direction: this.sortDirection() }, event as Sort);
    this.page.set(next.page); this.sort.set(next.sort); this.sortDirection.set(next.direction); this.load();
  }
  updateFilter(event: FilterValueEvent): void { this.filterValues.update((values) => ({ ...values, [event.key]: event.value })); }
  ...
  clearFilters(): void { this.filterValues.set({}); this.filterDisplayValues.set({}); this.filterOptions.set({}); this.applyFilters(); }
  updateAutocomplete(event: FilterAutocompleteEvent): void {
    ...
    const endpoint = this.definition.filterOptionsEndpoint?.() ?? `${this.definition.endpoint().split('?')[0]}/filter-options`;
    this.http.get<FilterOption[]>(endpoint, { params: { field: filter.key, query } }).subscribe({
      next: (options) => this.filterOptions.update((values) => ({ ...values, [filter.key]: options })),
  ...
  selectFilterOption(event: FilterValueEvent & { label: string }): void {
    ...
    this.updateFilter(event); this.applyFilters();
  }
  handleAction(event: DataTableActionEvent): void {
    const record = this.records().find((candidate) => this.definition.identifier(candidate) === event.rowId);
    if (!record) return;
  ...
  protected save(record: ResourceRecord | undefined, body: unknown): void {
    ...
    const request = record ? this.http.put(`${endpoint}/${this.definition.identifier(record)}`, body) : this.http.post(endpoint, body);
    request.subscribe({ next: () => this.load(), error: () => this.error.set(true) });
  }
  private actions(record: ResourceRecord): DataTableRowAction[] {
    const actions: DataTableRowAction[] = [];
    if (this.definition.canView?.()) actions.push({ key: 'view', label: this.definition.actionLabels?.view ?? 'RESOURCE.VIEW', icon: 'visibility' });
    if (this.canManage()) {
      actions.push({ key: 'edit', label: this.definition.actionLabels?.edit ?? 'RESOURCE.EDIT', icon: 'edit' });
      if (this.definition.canDelete?.(record) ?? true) actions.push({ key: 'delete', label: this.definition.actionLabels?.delete ?? 'RESOURCE.DELETE', icon: 'delete_outline', destructive: true });
    }
    return actions;
  }
}
