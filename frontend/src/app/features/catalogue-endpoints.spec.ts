...
describe('catalogue feature endpoints', () => {
  const cases: [Type<unknown>, string][] = [[SpecialitiesComponent, '/api/specialities'], [AddressesComponent, '/api/addresses'], [CountriesComponent, '/api/countries'], [AdministrativeDivisionsComponent, '/api/administrative-divisions']];
  ...
  it('surfaces the continent lookup error without opening the country dialog', () => {
    ...
    TestBed.configureTestingModule({ providers: [provideHttpClient(), provideHttpClientTesting(), provideTranslateService(), { provide: MatDialog, useValue: dialog }, { provide: AuthService, useValue: { activeMembership: signal(null), hasPermission: () => true } }] });
    const component = TestBed.runInInjectionContext(() => new CountriesComponent());
    const http = TestBed.inject(HttpTestingController);
    http.expectOne((candidate) => candidate.url === '/api/countries').flush({ content: [], page: 0, size: 10, totalElements: 0, totalPages: 0 });
    component.create();
    http.expectOne('/api/countries/continents').flush('failure', { status: 500, statusText: 'Server error' });
  ...
  it('reloads localized catalogue data when the language changes', () => {
    ...
    const http = TestBed.inject(HttpTestingController);
    http.expectOne((candidate) => candidate.url === '/api/specialities').flush({ content: [], page: 0, size: 10, totalElements: 0, totalPages: 0 });
  ...
  it('renders the localized display name while preserving the canonical speciality name', () => {
    ...
    const http = TestBed.inject(HttpTestingController);
    http.expectOne((candidate) => candidate.url === '/api/specialities').flush({ content: [{ id: 1, name: 'Pediatric Dentistry', displayName: 'Odontopediatria', procedures: [] }], page: 0, size: 10, totalElements: 1, totalPages: 1 });
