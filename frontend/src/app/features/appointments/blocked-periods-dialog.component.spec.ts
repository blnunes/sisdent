...
describe('BlockedPeriodsDialogComponent', () => {
  const api = { blockedPeriods: vi.fn(), createBlockedPeriod: vi.fn(), updateBlockedPeriod: vi.fn(), deleteBlockedPeriod: vi.fn() };
  const ref = { close: vi.fn(), keydownEvents: () => EMPTY };
  const dialog = { open: vi.fn(() => ({ afterClosed: () => of(true) })) };
  const data = { organizationId: 'organization-1', clinicUnitId: 'clinic-1', timezone: 'Europe/Lisbon', practitioners: [], from: '2026-03-29T00:00:00Z', to: '2026-03-30T00:00:00Z', onFinished: vi.fn() };
  ...
  it('uses the finite selected-clinic range without rendering opaque management fields', () => {
    ...
    expect(api.blockedPeriods).toHaveBeenCalledWith(data.organizationId, data.clinicUnitId, data.from, data.to);
  ...
  it('submits clinic-local input as UTC and refreshes safely', () => {
    ...
    component.save();
    expect(api.createBlockedPeriod).toHaveBeenCalledWith(data.organizationId, expect.objectContaining({ clinicUnitId: data.clinicUnitId, startAt: expect.stringMatching(/Z$/), endAt: expect.stringMatching(/Z$/) }));
  ...
  it('keeps stale-version and authorization failures generic while refreshing the preserved range', () => {
    api.createBlockedPeriod.mockReturnValue(throwError(() => ({ code: 'CONFLICT', message: 'raw stale version' })));
    const component = TestBed.createComponent(BlockedPeriodsDialogComponent).componentInstance;
  ...
  it('uses internal opaque identifiers and versions for update/delete only', () => {
    const period = { globalId: '11111111-1111-4111-8111-111111111111', clinicUnitId: data.clinicUnitId, practitionerId: null, startAt: '2026-03-29T09:00:00Z', endAt: '2026-03-29T10:00:00Z', version: 4 };
    ...
    component.save();
    expect(api.updateBlockedPeriod).toHaveBeenCalledWith(data.organizationId, period.globalId, period.version, expect.any(Object));
    component.remove(period);
    expect(api.deleteBlockedPeriod).toHaveBeenCalledWith(data.organizationId, data.clinicUnitId, period.globalId, period.version);
  });
});
