...
import { AdministrativeDivisionGraphqlService, AdministrativeDivisionWrite } from '../../core/administrative-division-graphql.service';
...
export class AdministrativeDivisionsComponent extends CatalogueListController {
  ...
  constructor() { super({ endpoint: () => '', maintainPermission: 'MAINTAIN_ADMINISTRATIVE_DIVISIONS', columns: COLUMNS, identifier: (record) => Number(record['id']), primary: (record) => String(record['name'] ?? '—'), cells: divisionCells }, { fields: [{ key: 'name', label: 'Name', required: true }, { key: 'code', label: 'Code', required: true }, { key: 'type', label: 'Type', required: true }, { key: 'countryCode', label: 'Country code', required: true }], fromRecord: (record) => ({ name: String(record['name'] ?? ''), code: String(record['code'] ?? ''), type: String(record['type'] ?? ''), countryCode: String(nested(record, 'country')['code'] ?? '') }), toRequest: (values) => values, title: (editing) => editing ? 'Edit administrative division' : 'New administrative division' }); this.load(); }
  override load(): void { this.loading.set(true); this.divisions.list(this.page(), this.pageSize(), this.sort(), this.sortDirection()).subscribe({ next: (response) => { this.records.set(response.content); this.totalElements.set(response.totalElements); this.loading.set(false); }, error: () => { this.error.set(true); this.loading.set(false); } }); }
  protected override save(record: ResourceRecord | undefined, body: unknown): void { this.divisions.save(record as never, body as AdministrativeDivisionWrite).subscribe({ next: () => this.load(), error: () => this.error.set(true) }); }
  protected override remove(record: ResourceRecord): void { if (!confirm(this.translate.instant('RESOURCE.DELETE_CONFIRM', { name: String(record['name'] ?? '—') }))) return; this.divisions.delete(String(record['id'])).subscribe({ next: () => this.load(), error: () => this.error.set(true) }); }
}
