...
@Component({
  ...
  imports: [MatButtonModule, MatCardModule, MatCheckboxModule, MatExpansionModule, MatFormFieldModule,
    ...
    MatSnackBarModule, MatToolbarModule, TranslatePipe, LanguageSelectorComponent, ThemeToggleComponent],
...
export class PermissionsComponent {
  ...
  save(): void { const u = this.selected(); if (!u || u.role === 'ADMIN' || !this.hasChanges()) return; this.saving.set(true); this.api.updatePermissions(u.id, [...(this.drafts.get(u.id) ?? [])]).subscribe({ next: () => { this.saving.set(false); this.snack.open(this.translate.instant('PERMISSIONS_PAGE.SAVE_SUCCESS'), this.translate.instant('USERS.CLOSE'), { duration: 3500 }); this.load(); }, error: () => { this.saving.set(false); this.snack.open(this.translate.instant('PERMISSIONS_PAGE.SAVE_ERROR'), this.translate.instant('USERS.CLOSE'), { duration: 5000, panelClass: 'error-snackbar' }); } }); }
