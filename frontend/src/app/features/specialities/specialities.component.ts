...
const COLUMNS: readonly DataTableColumn[] = [{ key: 'name', label: 'SPECIALITIES.TABLE.NAME', sortable: true }, { key: 'procedures', label: 'SPECIALITIES.TABLE.PROCEDURES' }, { key: 'actions', label: '' }];
...
export class SpecialitiesComponent extends ResourceListController {
  ...
  constructor() {
    super({ endpoint: () => '/api/specialities', maintainPermission: 'MAINTAIN_SPECIALITIES', columns: COLUMNS, filters: FILTERS, identifier: (record) => Number(record['id']), primary: (record) => String(record['displayName'] ?? record['name'] ?? '—'), cells: specialityCells });
    this.load();
  }
  ...
  private openSpecialityEditor(record?: ResourceRecord): void {
    this.dialog.open(SpecialityFormDialogComponent, { width: '820px', maxWidth: '94vw', maxHeight: '92vh', autoFocus: 'first-tabbable', data: { title: record ? 'SPECIALITIES.FORM.EDIT_TITLE' : 'SPECIALITIES.FORM.NEW_TITLE', name: record ? String(record['name'] ?? '') : undefined, translations: record?.['translations'] as Record<string, string> | undefined, procedures: record ? parseProcedures(JSON.stringify(record['procedures'] ?? [])) : [] } }).afterClosed().subscribe((result?: SpecialityFormResult) => { if (result) this.save(record, result); });
  }
}
function specialityCells(record: ResourceRecord, names: CatalogDisplayNameService): Readonly<Record<string, string>> { return { name: names.speciality(record), procedures: ((record['procedures'] as ResourceRecord[] | undefined) ?? []).map((item) => names.procedure(item)).filter(Boolean).join(', ') || '—' }; }
