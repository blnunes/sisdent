...
@Component({ selector: 'app-accounts', standalone: true, imports: [ReactiveFormsModule, MatButtonModule, MatCheckboxModule, MatCardModule, MatChipsModule, MatIconModule, MatFormFieldModule, MatInputModule, MatSelectModule, MatProgressSpinnerModule, MatSidenavModule, TranslatePipe, AppHeaderComponent, ModuleNavigationComponent], templateUrl: './accounts.component.html', styleUrl: './accounts.component.scss' })
export class AccountsComponent {
  ...
  readonly loading = signal(true); readonly error = signal(''); readonly accounts = signal<AccountSummary[]>([]); readonly manageableOrganizations = signal<OrganizationOption[]>([]); readonly viewAllAccounts = signal(false);
  ...
  load(): void { this.loading.set(true); this.error.set(''); const activeMembership = this.auth.activeMembership(); if (this.auth.isPlatformAdministrator()) { const page = this.viewAllAccounts() || !activeMembership ? this.api.listPlatform() : this.api.listOrganization(activeMembership.organizationId); forkJoin({ page, organizations: this.api.listPlatformOrganizations() }).subscribe({ next: ({ page, organizations }) => { this.accounts.set(page.content); this.manageableOrganizations.set(organizations); this.loading.set(false); }, error: () => { this.error.set(this.translate.instant('ACCOUNTS.LOAD_ERROR')); this.loading.set(false); } }); return; } const managementOrganizationId = this.auth.session()?.accountManagementOrganizationId; if (!activeMembership || activeMembership.organizationId !== managementOrganizationId || activeMembership.role !== 'ORGANIZATION_ADMIN' || activeMembership.clinicUnitId) { this.error.set(this.translate.instant('ACCOUNTS.NO_MANAGEMENT_ACCESS')); this.loading.set(false); return; } const organization = { id: activeMembership.organizationId, name: activeMembership.organizationName, active: true }; this.manageableOrganizations.set([organization]); this.api.listOrganization(organization.id).subscribe({ next: page => { this.accounts.set(page.content); this.loading.set(false); }, error: () => { this.error.set(this.translate.instant('ACCOUNTS.LOAD_ERROR')); this.loading.set(false); } }); }
